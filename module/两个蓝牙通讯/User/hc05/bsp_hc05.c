...
{		
    ...
    /*开启GPIO时钟*/
    BLT_INT_GPIO_CLK_ENLAB();
    BLT_KEY_GPIO_CLK_ENLAB();
    
    /*配置GPIO
    *INT -- 浮空输入 -- 判断是否模块连接
    *KEY -- 推挽输出 -- 决定是否模块进入AT模式
    */
    GPIO_InitStructure.Pin = BLT_INT_GPIO_PIN;	
    GPIO_InitStructure.Mode = GPIO_MODE_INPUT; 
    GPIO_InitStructure.Pull = GPIO_NOPULL;
    GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH; 
    HAL_GPIO_Init(BLT_INT_GPIO_PORT, &GPIO_InitStructure);	
    
    GPIO_InitStructure.Pin = BLT_KEY_GPIO_PIN;
    GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH; 
    HAL_GPIO_Init(BLT_KEY_GPIO_PORT, &GPIO_InitStructure);	
